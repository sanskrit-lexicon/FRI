"""
Markup fixer + audit for fri.txt (FRI).
Counterpart of PWG#175, PWK#113, and siblings.

FRI-specific notes: Only <lang> (23,013). No whitespace hits.

Inputs: ../../../csl-orig/v02/fri/fri.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "fri" / "fri.txt", HERE / "fri.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "fri_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for fri.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"FRI markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against fri.txt.\n")
        f.write(f"Paired tags: <lang>\n")
        f.write(f"Notes: Only <lang> (23,013). No whitespace hits.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <lang>\n\n")
